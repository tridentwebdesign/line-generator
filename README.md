...
- **17項目のリアルタイムパラメーター** — スライダー操作で即時反映
...
| Curl | ラインの内巻き（−）・外巻き（+）。−1.0〜1.0 |
| Wiggle | 蛇のようなうねり（高周波の蛇行波）。0〜1.0 |
...
- 各レイヤーが独自のパラメーター・角度を保持

異なる設定のレイヤーを重ねたり、角度を変えたレイヤーを組み合わせると複雑なビジュアルを作れます。
...
各境界曲線は2ハーモニック波（基本波 + 倍音）で構成し、A と B の位相を約0.45rad ずらすことで収束・発散するライン束を生成します。**カール**は各ラインの中心からの距離に比例した弓形変位で内巻き・外巻きを表現し、**うねり**は高周波の正弦波でライン毎に位相をずらした蛇行を加えます。さらにスムーズノイズで完全な規則性を崩し、有機的な印象を与えます。
